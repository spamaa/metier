...
Rectangle {
  ...
  function getDisplayData(for_index) {
    ...
    if (contextRoot.ot_ActivityThread !== undefined) {
      ...
      //TODO: get an AccountActivity notary unitname and transaction id. 'From' can
      // also provide contact name which wont help with URL creation.
      //blockchainBrowserLink.depositChains = contextRoot.activityModel.depositChains
      blockchainBrowserLink.txid = display_data['from']
      blockchainBrowserLink.setWebURL()// generate explorer link button
  ...
  Column {
    ...
    MatterFi_BlockchainExplorerButton {
      ...
      //TODO: locate role/property for unitname/notary resolution for OT contact ActivityThread
      // also need the notary unitname or depositChains role.
      anchors.horizontalCenter: parent.horizontalCenter
  ...
  transitions: [
    Transition {
      from: "open"; to: "closed"
      ParallelAnimation {
        OpacityAnimator { target: contextRoot; duration: 400 }
      }
    },
    Transition {
      from: "closed"; to: "open"
