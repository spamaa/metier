...
Item {
	...
	property string contactID: "" // used to seed a random dawgs avatar.
	...
	//----------------------
	// Using 'contactID' string pick an index for both BG color and Face svg file.
	function seedPayentCodeFace() {
		var faceSeed = 0
		for (var i=0; i < contactID.length; i++) {
			var charValue = contactID.charCodeAt(i)
			faceSeed += charValue
		}
		...
		// debugger:
		//console.log("Dawgs Face:", faceName, bgcolor, faceSeed, contactID)
	}
