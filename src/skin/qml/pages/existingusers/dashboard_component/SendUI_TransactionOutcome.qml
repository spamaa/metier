...
Item {
  ...
  Component.onCompleted: {
    ...
    if (dashViewRoot.sendtoContactID !== "") {
      ...
      // opentxs::ui::implementation::ActivityThread::Pay: Invalid account: (<accountID>)
      ...
      //console.log("Sending funds via OTidentity.contactActivityThread_OTModel.pay()", OTidentity.focusedAccountActivity_OTModel.accountID, amountToSend_validated)
      ...
      //var payment_code = OTidentity.contactActivityThread_OTModel.paymentCode(OTidentity.focusedAccountActivity_OTModel.type)
      //console.log("SendUI Transaction for contact address payment:", payment_code) returns undefined value

      // make the account asset payment via contact: BugZilla#333
      //console.log("Sending funds via OTidentity.focusedAccountActivity_OTModel.sendToContact()", dashViewRoot.sendtoContactID, amountToSend_validated)
      ...
      // If any imediate issues with creating a transaction, no transaction index id will be returned.
      contextRoot.wasError = (dashViewRoot.txRequestIndex === undefined || dashViewRoot.txRequestIndex === false)
      ...
      console.log("Wating for responce of outcome for transaction sent to blockchain. index:", dashViewRoot.txRequestIndex)
      ...
      dashViewRoot.txRequestIndex = OTidentity.focusedAccountActivity_OTModel.sendToAddress(
        dashViewRoot.sendtoAddressID, amountToSend_validated, memo);
      //debugger:
      console.log("Sending via OTidentity.focusedAccountActivity_OTModel.sendToAddres()", dashViewRoot.sendtoAddressID, amountToSend_validated)
    }
  }
  ...
  //----------------------
  // Track recent activity transactions to watch for any send errors, For a 
  // complete check of transaction out come, have to wait for blockchain responce:
