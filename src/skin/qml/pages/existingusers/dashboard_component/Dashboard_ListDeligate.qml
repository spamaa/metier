...
Button {
  ...
  property alias blockchainIcon: blockchainIcon
  ...
  contentItem: Rectangle {
    ...
    Rectangle {
      ...
      MatterFi_BlockChainIcon {
        ...
        abvNotary: (visible ? contextRoot.notaryIcon : "")
        anchors.verticalCenter: parent.verticalCenter
      }

      Image {
        id: avatarImage
        visible: (contextRoot.imageSource.length > 0 && contextRoot.contactID === "")
      ...
      Dawgs_Avatar {
        ...
        avatarUrl: contextRoot.imageSource
        visible: (contextRoot.contactID.length > 0)
        contactID: contextRoot.contactID
        anchors.horizontalCenter: parent.horizontalCenter
      }
    }//end 'imageDisplayed'
  ...
  MouseArea {
    ...
    onClicked: {
      if (mouse.button === Qt.LeftButton) {
        contextRoot.leftClicked()
      } else {
        contextRoot.rightClicked()
      }
      //debugger:
      //console.log("Dash list deligate mouse activity:", mouse)
      //QML_Debugger.listEverything(mouse)
    }
  }
