...
Page {
  ...
  property string sendtoMatterCodeID: "" // Recieving payment code.

  property string useAccountID: ""       // When send funds was summoned, was an asset chosen already.

  // TODO: because of USD and block chain display values, it's assumed that one only passes
  // the block chain value amount ever when calling on AccountActivity.send()
  ...
  MatterFi_PinWall {
    ...
    onPinCorrectlyEntered: {
      if (pageRoot.passAlongData !== undefined) {
        ...
        } else if (pageRoot.passAlongData.sendMatterCode !== undefined) {
          // navigation from Contacts view
          dashViewRoot.sendtoMatterCodeID = pageRoot.passAlongData.sendMatterCode

          //TODO: its assumed here that the provided code is correct as it
          // has generally been provided by an OT model. Still, a way to double
