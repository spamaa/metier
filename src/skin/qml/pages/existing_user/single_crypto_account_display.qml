...
Page {
	...
	Component {
		...
		Rectangle {
			...
			Component.onCompleted: {
				...
				// this always defaults the for first start up to PKT notary asset.
				if (notaryname == "PKT") {
					...
					assetFocusSelection.currentIndex = index
					assetFocusSelection.displayedText = notaryname
					//transaction_display.accountActivityModel = OTidentity.focusedAccountActivity_OTModel
					//debugger: 
					//console.log("Single crypto view, displaying notary:", notaryname, index)
				}
			}
		}
	}
	...
	//-----------------------------------------------------------------------------
	// Show a drop down selection list for other avialable assets to view:
	property int assetListIndex: -1
	Row {
		...
		Text {
			id: assetContextSelection
			text: qsTr("Showing Asset:")
			color: CustStyle.accent_text
			font.pixelSize: CustStyle.fsize_normal
			anchors.verticalCenter: parent.verticalCenter
		}

		MatterFi_AssetComboBox {
			id: assetFocusSelection
			model: (accountlistView.model)
			anchors.verticalCenter: parent.verticalCenter
			onInteraction: {
				pageRoot.assetListIndex = assetFocusSelection.currentIndex
				// change send/reveive asset targets and transactions
				pageRoot.accountList_selection = selectionModel
				OTidentity.focusedAccountActivity_OTModel.syncProgressUpdated.disconnect(sync_status)
				OTidentity.setAccountActivityFocus(selectionModel.account)
				// update user on current focused asset sync progress
				syncDeligateAreaText.text = qsTr("Awaiting Blockchain...")
				percentageTextDisplay.text = ""
				bsyncPB.visible = false
				bsyncPB.isdone = false
				OTidentity.focusedAccountActivity_OTModel.syncProgressUpdated.connect(sync_status)
				//transaction_display.accountActivityModel = OTidentity.focusedAccountActivity_OTModel
				//debugger:
				//console.log("Asset selected for viewing:", assetFocusSelection.selectionModel)
				//console.log("Single crypto view, displaying notary:", selectionModel.notaryname, selectionModel.account)
				//QML_Debugger.listEverything(assetFocusSelection.selectionModel)
			}
		}
	}
	...
	function sync_status(current, total) {
		...
		syncDeligateAreaText.text = qsTr("Syncing Blockchain:")
		if (bsyncPB.value >= 1.0) {
			bsyncPB.isdone = true
			bsyncPB.visible = false
		} else {
			bsyncPB.isdone = false
			bsyncPB.visible = true
		}
	}
