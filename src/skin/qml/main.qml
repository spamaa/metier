...
/*
  ...
  utilize the 'otwrap' rootContext. Sample:

    otwrap.<function>.connect(qml_id.function_name)
...
*/
//-----------------------------------------------------------------------------
// https://doc.qt.io/archives/qt-5.10/scalability.html
...
Page {
    ...
    property bool isDebugging: false // First page is debug screen navigator?
    ...
    function firstpage() {
        if (rootAppPage.isDebugging) {
            rootAppPage.replacePage("pages/debug_navigator.qml")
    ...
    function focusWindowChanged(window_in_focus) {
        ...
        //console.log("GuiApp:", guiApplication) 
        //QML_Debugger.listEverything(guiApplication)
        ...
        rootAppPage.osDisplay_window.setTitle("pkt-desktop")
    ...
    function keyEvent_normal_window_size(key_event) {
        ...
        if (Qt.platform.os === "osx" || Qt.platform.os === "unix") {
            if (rootAppPage.osDisplay_window !== null) {
                //console.log(rootAppPage.osDisplay_window.visibility)
    ...
    Component.onCompleted: {
        ...
        app.displayBlockchainChooser.connect(rootAppPage.display_blockChainChooser)
        app.displayFirstRun.connect(rootAppPage.display_firstRun)
        app.displayMainWindow.connect(rootAppPage.display_mainWindow)
        app.displayNamePrompt.connect(rootAppPage.display_namePrompt)
        // get the focuse QWindow object for things like MenuBar
        guiApplication.focusWindowChanged.connect(rootAppPage.focusWindowChanged)
        // debugger:
    ...
    Component.onDestruction: {
        if (osDisplay_window !== null) {
            rootAppPage.osDisplay_window.visibilityChanged.disconnect(rootAppPage.visibility_changed)
            rootAppPage.osDisplay_window.onActiveFocusItemChanged.disconnect(rootAppPage.onActiveFocusItemChanged)
            guiApplication.focusWindowChanged.disconnect(rootAppPage.focusWindowChanged)
        }
        if (app !== null) {
            app.displayBlockchainChooser.disconnect(rootAppPage.display_blockChainChooser)
            app.displayFirstRun.disconnect(rootAppPage.display_firstRun)
            app.displayMainWindow.disconnect(rootAppPage.display_mainWindow)
            app.displayNamePrompt.disconnect(rootAppPage.display_namePrompt)
        }
    }
