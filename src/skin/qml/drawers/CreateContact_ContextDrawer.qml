...
Drawer {
  ...
  property var contactList_model: ({})
  property var accountActivity_model: ({})
  ...
  //-----------------------------------------------------------------------------
  // Called on first opening of drawer:
  function setup(contactList_model, accountActivity_model) {
    drawerRoot.contactList_model = contactList_model
    drawerRoot.accountActivity_model = accountActivity_model
    console.log("Create contact drawer was propagated.", contactList_model, accountActivity_model)
  }
  ...
  function newContact(contact_name, address) {
      ...
      var addressValid = drawerRoot.accountActivity_model.validateAddress(address)
      if (addressValid) {
        console.log("Adding Contact into the List:", contact_name, address)
        //otwrap.addContact(contact_name, address)
        drawerRoot.contactList_model.addContact(contact_name, address)
        drawerRoot.created_new_contact = true
        contextFunctionTextDescription.text = qsTr("Contact Created:")
      } else {
        console.log("New Contact address is not valid.", address)
